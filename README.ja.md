...
- **メモリ使用量の最適化**：v0.3.0 ではツールページ、CodeMirror エディター、ログ、永続化状態、正規表現結果をより控えめに読み込み・保持します。
...
- CodeMirror エディターは初回利用時まで遅延読み込みし、ツールのアンマウント時にエディタービューとサイズ監視を明示的に解放します。
...
- **Visuals**：**Base UI** と **TailwindCSS** で構築
