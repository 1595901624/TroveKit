...
<div align="center">
<a href="https://github.com/1595901624/trovekit/releases"><img src="https://img.shields.io/github/v/release/1595901624/Trovekit?display_name=tag&label=version&color=blue" alt="Version"></a>
...
- **操作快**：輸入即見結果（支援即時更新）
- **可追溯**：帶操作日誌與一鍵複製
- **跨平台**：Windows 10/11（32 位和 64 位）、macOS 10.13+（Intel 與 Apple Silicon）、Linux 桌面（推薦 Ubuntu 24.04 x64/arm，需 WebKit2GTK ≥4.1）。
...
- **正則工具**：即時正則測試，支援語法高亮、匹配組與旗標（flags）設置（已在 v0.3.0 中最佳化）。
- **記憶體佔用最佳化**：v0.3.0 對工具頁、CodeMirror 編輯器、日誌、持久化狀態和正則結果採用更保守的按需載入與保留策略。
...
- **網段計算**：支援 IPv4 CIDR 與子網掩碼轉換，計算網路位址、廣播位址、主機範圍和主機數量。
- **JSON ↔ XML**：雙向轉換，即時處理
- **JSON ↔ YAML**：雙向轉換，即時處理
- 多種格式語法高亮
- 範例資料支援，快速測試
...
- 工具頁和同級 Tab 改為真正按需掛載，減少隱藏元件樹和編輯器實例常駐。
- CodeMirror 編輯器延遲到首次使用時載入；工具卸載時會明確釋放編輯器視圖和尺寸觀察器。
- 工具狀態只寫入 Tauri Store；舊 `localStorage` 狀態遷移一次後刪除，避免大型文字被重複快取。
- 正則匹配結果最多渲染 1000 條，批次替換計數不再額外建立完整匹配陣列。
...
- **Theme**：深淺色主題，支援系統同步
- **Visuals**：基於 **Base UI** 與 **TailwindCSS** 建構
...
- pnpm
- Rust（stable）
- Tauri v2 依賴（不同系統要求略有差異；若首次建構失敗，請按 Tauri 官方文件安裝系統依賴）。Linux 需要安裝 WebKit2GTK 4.1 或更高版本（Ubuntu 24.04 內建）。
