    ...
    <HashWorkbench
      ...
      configContent={(
        <div className="grid gap-2 [grid-template-columns:repeat(auto-fit,minmax(min(100%,15rem),1fr))]">
          <div className="grid min-w-0 grid-cols-[minmax(0,1fr)_88px] gap-2"><Input size="sm" label={t("tools.hash.key")} placeholder={t("tools.hash.keyPlaceholder")} value={desKey} onValueChange={setDesKey} className="min-w-0" classNames={{ inputWrapper: "h-9 min-h-9 bg-background" }} /><Select size="sm" label={t("tools.hash.type")} selectedKeys={[desKeyType]} onChange={(event) => setDesKeyType(event.target.value)}><SelectItem key="text">{t("tools.hash.text")}</SelectItem><SelectItem key="hex">{t("tools.hash.hex")}</SelectItem></Select></div>
          <div className="grid min-w-0 grid-cols-[minmax(0,1fr)_88px] gap-2"><Input size="sm" label={t("tools.hash.iv")} placeholder={t("tools.hash.iv")} value={desIv} onValueChange={setDesIv} isDisabled={desMode === "ECB"} className="min-w-0" classNames={{ inputWrapper: "h-9 min-h-9 bg-background" }} /><Select size="sm" label={t("tools.hash.type")} selectedKeys={[desIvType]} onChange={(event) => setDesIvType(event.target.value)} isDisabled={desMode === "ECB"}><SelectItem key="text">{t("tools.hash.text")}</SelectItem><SelectItem key="hex">{t("tools.hash.hex")}</SelectItem></Select></div>
